...
public class EquipmentToolTip extends ToolTip {
    ...
    private var roftext:TextFieldDisplayConcrete;
    private var cdtext:TextFieldDisplayConcrete;
    ...
    private function addCdToEffectsList():void {
        if (((this.objectXML.hasOwnProperty("Cooldown")) && (!(this.comparisonResults.processedTags[this.objectXML.Cooldown.toXMLString()]) == true))) {
            if (!this.comparisonResults.processedTags[this.objectXML.Cooldown.toXMLString()]) {
                this.effects.push(new Effect("Cooldown: {cd} Seconds" ,  {"cd": this.objectXML.Cooldown}));
            }
        }
    }
            ...
            if (this.comparisonResults.processedTags[_local1.toXMLString()] != true) {
                ...
                switch (_local6) {
                    ...
                    case ActivationType.HEAL:
                        ...
                        break;
                    case ActivationType.HEAL_SUB:
                        var num:int = 100 - _local1.@percentage * 100;
                        this.effects.push(new Effect(TextKey.PERCENT_SUB, {
                            "percentage": (( + num) + "% HP")
                        }));
                        break;
                    ...
                    case ActivationType.POISON_GRENADE:
                        ...
                        break;
                    case ActivationType.HealingGrenade:
    ...
    private function makeRestrictionList():void {
        ...
        if (this.objectXML.hasOwnProperty("MC")) {
            this.restrictions.push(new Restriction("Speciality: Crafted", 0x17d1d4, true));
        }
        if (this.objectXML.hasOwnProperty("END")) {
            this.restrictions.push(new Restriction("Speciality: End", 0x6a0dad, true));
        }

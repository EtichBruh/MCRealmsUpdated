    ...
    public class Database : IDisposable
    {
        ...
        public DbAccount CreateGuestAccount(string uuid)
        {
            ...
            var acnt = new DbAccount(_db, 0)
            {
                ...
                //Gems = newAccounts.Gems,
                //TotalGems = newAccounts.Gems,
                PassResetToken = ""
            };
        ...
        private static readonly Dictionary<CurrencyType, string[]> CurrencyKey = new Dictionary<CurrencyType, string[]>
        {
            ...
            //{ CurrencyType.Gems, new [] { "totalGems", "Gems" } }
        };
        ...
        //public Task UpdateGems(DbAccount acc, int amount, ITransaction transaction = null)
        //{
        //    var trans = transaction ?? _db.CreateTransaction();
        //
        //    if (amount > 0)
        //        trans.HashIncrementAsync(acc.Key, "totalGems", amount)
        //            .ContinueWith(t =>
        //            {
        //                if (!t.IsCanceled)
        //                    acc.TotalGems = (int)t.Result;
        //            });
        //
        //    var task = trans.HashIncrementAsync(acc.Key, "Gems", amount)
        //        .ContinueWith(t =>
        //        {
        //            if (!t.IsCanceled)
        //                acc.TotalGems = (int)t.Result;
      ...
      //  }
        ...
        public int? CreateCharacterSlot(DbAccount acc, ITransaction tran = null)
        {
            if (tran == null)
                return (int)_db.HashIncrement(acc.Key, "maxCharSlot");
            tran.HashIncrementAsync(acc.Key, "maxCharSlot");
            return null;
        }
        ...
        public void WipeAccount(DbAccount acc, XmlData dat, string killer)
        {
            ...
            _db.HashSetAsync(acc.Key, "Gems", 0, flags: CommandFlags.FireAndForget);
            _db.HashSetAsync(acc.Key, "totalGems", 0, flags: CommandFlags.FireAndForget);

    ...
    public class Loot : List<MobDrops>
    {
                ...
                var loot = new List<Item>();
                    foreach (var i in possibleDrops)
                    {
                        if (i.Threshold > 0 && i.Threshold <= player.Item2 &&
                            Rand.NextDouble() < i.Probabilty * lootDropBoost * luckStatBoost * rankBoost)
                        {
                            loot.Add(i.Item);
                            reqDrops[i]--;
                        }
        ...
        private static void ShowBags(Enemy enemy, IEnumerable<Item> loots, params Player[] owners)
        {
            ...
            foreach (var i in loots)
            {
                ...
                if (i.Common)
                {
                    enemy.Owner.WorldAnnouncement($"A faggot called {owners[0].Name} has looted a common item: {i.ObjectId}");
                }
                if (i.Uncommon)
                {
                    enemy.Owner.WorldAnnouncement($"A faggot called {owners[0].Name} has looted a uncommon item: {i.ObjectId}");
                }
                if (i.Rare)
                {
                    enemy.Owner.WorldAnnouncement($"A faggot called {owners[0].Name} has looted a rare item: {i.ObjectId}");
                }
                if (i.Epic)
                {
                    enemy.Owner.Manager.Chat.LootAnnounce($"A faggot called {owners[0].Name} has looted a epic item: {i.ObjectId}");
                }
                if (i.Legendary)
                {
                    //owners[0].Client.SendPacket(new GlobalNotification() { Text = "nwordpass" });
                    enemy.Owner.Manager.Chat.LootAnnounce($"A faggot called {owners[0].Name} has looted a legendary item: {i.ObjectId}");
                }
                if (i.Raid)
                {
                    enemy.Owner.Manager.Chat.LootAnnounce($"A faggot called {owners[0].Name} has looted a raid item: {i.ObjectId}");
                }

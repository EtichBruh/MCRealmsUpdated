...
namespace wServer.networking.handlers
{
    class HelloHandler : PacketHandlerBase<Hello>
    {
        ...
        private DbAccount VerifyConnection(Client client, Hello packet)
        {
            ...
            if (packet.Size == 10147063) //update every time before updating server on vps
            {
                Program.DL.LogToDiscord(WebHooks.ClientLog, "Client Log", $"[{acc.Name}'s client size has {packet.Size} bytes.");
                return acc;
            }
            else
            Program.DL.LogToDiscord(WebHooks.ModifiedClientLog, "Client Log", $"[{acc.Name} is using a modified client, /hide and /visit {acc.Name} to see if they're hacking.");
            Program.DL.LogToDiscord(WebHooks.ClientLog, "Client Log", $"[{acc.Name}'s client size has {packet.Size} bytes.");

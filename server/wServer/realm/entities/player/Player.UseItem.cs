...
namespace wServer.realm.entities
{
    partial class Player
    {
            ...
            foreach (var eff in item.ActivateEffects)
            {
                switch (eff.Effect)
                {
                    ...
                    case ActivateEffects.Heal:
                        ...
                        break;
                    case ActivateEffects.HealSub:
                        AEHealSub(time, item, target, eff);
                        break;
                    ...
                    case ActivateEffects.Fame:
                        ...
                        break;
                    //case ActivateEffects.Gems:
                    //    AEAddGems(time, item, target, eff);
                    //    break;
                    ...
                    case ActivateEffects.MysteryDyes:
                        AEMysteryDyes(time, item, target, eff);
                        break;
                    case ActivateEffects.IncrementPercentStat:
                        AEIncrementPercentStat(time, item, target, eff);
                        break;
                    case ActivateEffects.Alarm:
                        AEAlarm(time, item, target, eff);
                        break;
                    case ActivateEffects.FlashBang:
                        AEFlashBang(time, item, target, eff);
                        break;
                    case ActivateEffects.WitherSkull:
                        AEWitherSkull(time, item, target, eff);
                        break;
                    case ActivateEffects.Mushroom:
                        AEMushroom(time, item, target, eff);
                        break;
                    case ActivateEffects.Glowstone:
                        AEGlowstone(time, item, target, eff);
                        break;
        ...
        private void AEMysteryDyes(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var availableSlot = Inventory.GetAvailableInventorySlot(item);
            var gameData = Manager.Resources.GameData;
            Inventory[availableSlot] = gameData.Items.Where(kvp => kvp.Value.Legendary).RandomElement(Random).Value;
            return;
        }

        private void AEMushroom(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            switch(item.ObjectId)
            {
                case "Super reD Mushroom":
                    SetDefaultSize(150);
                    break;

                case "Super Greeen Mushroom":
                    SetDefaultSize(50);
                    break;
            }
            return;
        ...
        private void AEGlowstone(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var gameData = Manager.Resources.GameData;

            if (item.ObjectId == "Glowstone")
            {
                foreach (var i in Owner.Enemies.Values.Where(e => e.ObjectDesc.ObjectId == "Aether Portal 1" || e.ObjectDesc.ObjectId == "Aether Portal 2" || e.ObjectDesc.ObjectId == "Aether Portal 3" || e.ObjectDesc.ObjectId == "Aether Portal 4" || e.ObjectDesc.ObjectId == "Aether Portal 5"))
                {
                    i.Death(time);
                }
            }
            else if (item.ObjectId == "Magical Bucket")
            {
                foreach (var i in Owner.Enemies.Values.Where(e => e.ObjectDesc.ObjectId == "Aether Portal 6"))
                {
                    i.Death(time);
                }
            }
        ...
        private void AEWitherSkull(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var gameData = Manager.Resources.GameData;

            foreach (var i in Owner.Enemies.Values.Where(e => e.ObjectDesc.ObjectId == "Wither Statue 0" || e.ObjectDesc.ObjectId == "Wither Statue 1" || e.ObjectDesc.ObjectId == "Wither Statue 2" || e.ObjectDesc.ObjectId == "Wither Statue 3"))
            {
                i.Death(time);
            }
        }

        private void AEAlarm(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            string[] portalStuff = { "enderRoar", "End Portal" };
            float[] pos = { 53f, 48f };

            switch (eff.Id)
            {
                default: // default to end raid in case something goes wrong
                    portalStuff = new string[] { "enderRoar", "End Portal" };
                    pos = new float[] { 45f, 32f };
                    break;
                case "Allah":
                    portalStuff = new string[] { "allah", "Allah Portal" };
                    pos = new float[] { 46f, 33f };
                    break;
                case "Wither Storm":
                    portalStuff = new string[] { "witherStorm", "Wither Storm Portal" };
                    pos = new float[] { 49f, 32f };
                    break;
                case "Thanos":
                    portalStuff = new string[] { "thanosRaid", "Thanos Portal" };
                    pos = new float[] { 48f, 33f };
                    break;
                case "Santa":
                    portalStuff = new string[] { "santaRaid", "Santa Portal" };
                    pos = new float[] { 49f, 36f };
                    break;
                case "Viktor":
                    portalStuff = new string[] { "Viktor", "Viktor Portal" };
                    pos = new float[] { 55f, 4f };
                    break;
            }

            if (Owner is Nexus)
            {
                foreach (var i in Manager.Clients.Keys.Where(x => x.Player != null).Select(x => x.Player))
                {
                    i.Client.SendPacket(new GlobalNotification() { Text = portalStuff[0] });
                }

                var gameData = Manager.Resources.GameData;
                ushort objType;
                if (!gameData.IdToObjectType.TryGetValue(portalStuff[1], out objType))
                    return;

                var entity = Resolve(Manager, objType);
                entity.Move(pos[0], pos[1]);
                Owner.EnterWorld(entity);

                Owner.Timers.Add(new WorldTimer(30000, (world, t) =>
                {
                    world.LeaveWorld(entity);
                }));


                Manager.Chat.Announce("A raid has started at the spawn of nexus! Closing in 30 seconds.");
                return;
            }
            else
                SendError("Must be popped in nexus faggot no refund for idiots!");
        }

        private void AEFlashBang(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            foreach (var plr in Owner.Players.Values.Where(p => p.DistSqr(p) < 10))
            {
                plr.Client.SendPacket(new GlobalNotification() { Text = "flashBang" });
            }
        }
        ...
        //private void AEAddGems(RealmTime time, Item item, Position target, ActivateEffect eff)
       // {
       //     if (Owner is Test || Client.Account == null)
        //        return;
//
 //           var acc = Client.Account;
   //         var trans = Manager.Database.Conn.CreateTransaction();
     //       Manager.Database.UpdateCurrency(acc, eff.Amount, CurrencyType.Gems, trans)
       //         .ContinueWith(t =>
        //        {
        //            CurrentGems = acc.Gems;
        // /       });
        //    trans.Execute(CommandFlags.FireAndForget);
        //}
        ...
        private void AEAddGold(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            ...
            var acc = Client.Account;
            var trans = Manager.Database.Conn.CreateTransaction();
            Manager.Database.UpdateCurrency(acc, eff.Amount, CurrencyType.Gold, trans)
                .ContinueWith(t =>
                {
                    Credits = acc.Credits;
                });
        ...
        private void AECreate(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            ...
            if (Owner is GuildHall || Owner is Vault) {
            ...
            {
                SendError("wtf nigga do it in vault or guild no refund faggor");
            }
        }
        ...
        private void AEIncrementPercentStat(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var idx = StatsManager.GetStatIndex((StatsType)eff.Stats);
            float playerStat = Stats.Base[idx];
            var amount = playerStat * eff.Percentage;
            var convert = Math.Round(amount);
            int result = Convert.ToInt32(convert);

            Stats.Base[idx] += result;
        }
        ...
        private void AEHealSub(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var idx = StatsManager.GetStatIndex((StatsType)eff.Stats);
            float playerHP = HP;
            var amount = playerHP * eff.Percentage;
            var convert = Math.Round(amount);
            int result = Convert.ToInt32(convert);
            if (!HasConditionEffect(ConditionEffects.Sick))
            {

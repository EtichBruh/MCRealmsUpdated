...
namespace wServer.realm
{
    public class ChatManager : IDisposable
    {
        ...
        private void SendTextPacket(Player src, Text tp, Predicate<Player> conditional)
        {
            ...
            Program.DL.LogToDiscord(WebHooks.ChatLog, "Chat Log", $"[{src.Owner.Name}({src.Owner.Id}){(filtered ? " *filtered*" : "")}] <{src.Name}> {tp.Txt}");
        ...
        public void LootAnnounce(string text, bool local = false)
        {
            if (string.IsNullOrWhiteSpace(text))
                return;
            ...
            if (local)
            {
                ...
                .Select(x => x.Player))
                {
                    i.AnnouncementLootReceived(text);
                }
                return;
            }
        ...
        public void AnnounceLoot(string text, bool local = false)
        {
            if (string.IsNullOrWhiteSpace(text))
                return;

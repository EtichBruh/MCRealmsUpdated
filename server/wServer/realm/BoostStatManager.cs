    ...
    class BoostStatManager
    {
        ...
        private void ApplyEquipBonus(InventoryChangedEventArgs e)
        {
            for (var i = 0; i < 4; i++)
            {
                ...
                foreach (var x in _player.Inventory[i].StatsBoost2)
                    IncrementPercentBoost((StatsType)x.Key, x.Value);
            }
        }
        ...
        private void IncrementPercentBoost(StatsType stat, float amount)
        {
            var i = StatsManager.GetStatIndex(stat);
            float boost = _boost[i];
            var statamount = boost * amount;
            var convert = Math.Round(statamount);

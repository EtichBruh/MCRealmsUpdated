...
namespace wServer.realm.commands
{
    class SpawnCommand : Command
    {
        ...
        public SpawnCommand() : base("spawn", permLevel: 5) { }

        protected override bool Process(Player player, RealmTime time, string args)
        {
            var world = player.Owner;

            if (player.Rank < 90 && (!(world is Test) && !(world is Vault)))
            {
                player.SendError("Can only spawn in your vault or test worlds.");
                return false;
            }
        ...
        private bool SpawnJson(Player player, string json)
        {
            ...
            if (props.spawns != null)
                foreach (var spawn in props.spawns)
                {
                    ...
                    /*if (player.Client.Account.Rank <= 110 &&
                            desc.ObjectType == 0x01c7 ||
                            desc.ObjectType == 0x0504
                        )
                    {
                        player.SendError("Unknown entity!");
                        return false;
        ...
        private void NotifySpawn(Player player, string mob, int? num = null)
        {
            ...
            var notif = mob;
            if (num != null)
                notif = "Spawning a " + ((num > 1) ? num + " " : "") + mob + " in 3 seconds...";
    ...
    {
        public ClearSpawnsCommand() : base("clearspawn", permLevel: 25, alias: "cs") { }
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            if (player.Rank < 90)
            {
                ...
                {
                    player.SendError("Can only spawn in vault.");
                    return false;
                }
            }
    ...
    {
        public GuildRankCommand() : base("guildrank", alias: "grank", permLevel: 100) { }
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            {
                player.SendInfo("Usage: /guildrank (or /grank) <player name> <guild rank>");
                return false;
            }
    ...
    class GimmeCommand : Command
    {
        ...
        public GimmeCommand() : base("give", permLevel: 5) { }
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            if (player.Client.Account.Rank <= 50 &&
                (
            #region Banned Items
                 item.ObjectId.Equals("Viktor Map") ||
                 item.ObjectId.Equals("Santa Map") ||
                 item.ObjectId.Equals("Flash Bang") ||
                 item.ObjectId.Equals("Wither Skull") ||
                 item.ObjectId.Equals("Thanos Map") ||
                 item.ObjectId.Equals("Wither Storm Map") ||
                 item.ObjectId.Equals("Allah Map") ||
                 item.ObjectId.Equals("End Map") ||
                 item.ObjectId.Equals("Hitler Map") 
            #endregion
                 ))
            {
                player.SendError("Insufficient rank.");
    ...
    class BackpackCommand : Command
    {

        public BackpackCommand() : base("backpack", permLevel: 10) { }
    ...
    {
        public KillPlayerCommand() : base("kill", permLevel: 1) { }
    ...
    {
        public SizeCommand() : base("size", permLevel: 2) { }
    ...
    {
        public MaxCommand() : base("max", permLevel: 4) { }
    ...
    {
        public AdminCommand() : base("admin", alias:"editor", permLevel: 110) { }
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            {
                player.SendInfo("Usage: /admin <player name> <0 or 1> (false or true)");
                return false;
            }

            var name = args.Substring(0, index);
            var admin = int.Parse(args.Substring(index + 1));

            if (Database.GuestNames.Contains(name, StringComparer.InvariantCultureIgnoreCase))
            {
                player.SendError("Cannot admin unnamed accounts.");
                return false;
            }
            ...
            {
                player.SendError("Cannot admin self.");
                return false;
            }
            ...
            // kick player from server to set admin status
            foreach (var i in player.Manager.Clients.Keys)
                if (i.Account.Name.EqualsIgnoreCase(name))
                    i.Disconnect();

            // wipes account if you remove their admin status!
            if (acc.Admin && admin != 1)
            {
                // reset account
            ...
            player.SendInfo($"{acc.Name} {(acc.Admin ? "now" : "no longer")} has admin status.");
            return true;
        }
    }
    ...
    {
        public MusicCommand() : base("music", permLevel: 25) { }

        protected override bool Process(Player player, RealmTime time, string music)
        {
            var world = player.Owner;
            if (player.Rank < 70 && !(world is Test))
            {
    ...
    {
        public QuakeCommand() : base("quake", permLevel: 100) { }
    ...
    {
        public GlowCommand() : base("glow", permLevel: 2) { }
    ...
    {
        public LinkCommand() : base("link", permLevel: 25) { }
    ...
    {
        public UnLinkCommand() : base("unlink", permLevel: 25) { }
    ...
    class WargCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string name)
        {
            ...
            // WIP: FIX THIS!!!! (requires client changes too, but this was fixed in fiery realms)
            player.ApplyConditionEffect(ConditionEffectIndex.Hidden);
            player.ApplyConditionEffect(ConditionEffectIndex.Invincible);
